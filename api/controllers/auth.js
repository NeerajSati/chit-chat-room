...
const registerUserValidate = async (req, res, next) => {
  ...
  if (username.length > 15 || username.length < 3) {
    return res
      .status(400)
      .json({ success: false, msg: "Username can only have 3-15 characters!" });
  }
  if (!email) {
  ...
  if (!password || password.length < 8) {
    return res.status(400).json({
      success: false,
      msg: "Password should have atleast 8 characters!",
    });
  }
...
const registerUser = async (req, res) => {
  try {
    ...
    if (photo) {
      ...
      profilePic =
        constants.templateProfilePic[
          Math.floor(Math.random() * constants.templateProfilePic.length)
        ];
    }
    ...
    joinedGroups.push({
      ...
      isAdmin: false,
      lastSeen: new Date(),
      groupId: "64bce84d09e7d36dd89aaa4c",
    });
    await GroupMember.insertMany(joinedGroups);

    return res
      .status(200)
      .json({ success: true, token: jwtVerify.generateToken(savedUser) });
  } catch (err) {
    ...
    if (err && err.code === 11000) {
      ...
      if (err.keyPattern.email) {
        return res
          .status(400)
          .json({ success: false, msg: "Email already exists!", error: err });
      }
...
const loginUser = async (req, res) => {
  try {
    ...
    if (!userData) {
      return res
        .status(400)
        .json({ success: false, msg: "Email Id does not exist!" });
    }
    ...
    if (validatePassword) {
      ...
      return res
        .status(200)
        .json({ success: true, token: jwtVerify.generateToken(userData) });
    }
    return res
      .status(400)
